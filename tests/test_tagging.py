...
class GetCropImageCoordsTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_raises_not_found_when_scene_missing(self) -> None:
        ...
        with self.assertRaises(similar_sku_service.SceneImageNotFoundError):
            await service.get_crop_image_coords(scene_id=999, object_indexes=[0])

    async def test_preserves_original_index_for_each_requested_object(self) -> None:
        ...
        session = _FakeSimilarSkuSession(
            scene_row={"image_url": "/uploads/scene.png", "bbox_coord": coords}
        )
        service = similar_sku_service.SimilarSkuService(
            session=session,
            gemini_service=_FakeGeminiService(),
...
class OrchestrateSimilarSkusTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_response_object_index_matches_request_not_loop_position(
        ...
        gemini_service = _FakeGeminiService()
        service = similar_sku_service.SimilarSkuService(
            session=session, gemini_service=gemini_service, settings=self.settings
        )
        ...
        self.assertEqual(result.objects[0].bbox_coord, bbox_coord[2])
        self.assertEqual(result.objects[1].bbox_coord, bbox_coord[0])
    ...
    async def test_returns_empty_objects_when_no_indexes_requested(self) -> None:
        """object_indexes를 비워서 요청하면 크롭/임베딩 없이 빈 목록을 반환합니다."""
        scene_row = {
            "image_url": "/uploads/scene-images/scene.png",
            "bbox_coord": [{"xmin": 0.0, "ymin": 0.0, "xmax": 100.0, "ymax": 100.0}],
        }
