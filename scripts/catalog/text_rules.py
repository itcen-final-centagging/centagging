...
COLOR_KEYWORDS: dict[str, list[str]] = {
    ...
    "그레이": ["그레이", "그레이지", "실버", "은색"],
    "브라운": [
        "브라운", "카멜", "월넛", "멀바우", "헤이즐넛", "메이플", "오크",
        "티크", "애쉬", "내추럴", "내츄럴", "원목색", "모카", "초코",
    ],
    "레드": ["레드", "버건디", "와인"],
    "옐로우": ["옐로우", "머스타드", "노랑"],
    "블루": ["블루", "인디고", "청록"],
...
MATERIAL_KEYWORDS: dict[str, list[str]] = {
    ...
    "메쉬": ["메쉬", "매쉬"],
    "패브릭": [
        "패브릭", "아쿠아텍스", "원단", "린넨", "리넨", "면혼방",
        "폴리에스터", "극세사", "마이크로화이버", "쉐닐", "부클", "직물",
    ],
    "라탄": ["라탄", "위빙"],
    ...
    "유리": ["강화유리", "유리"],
    "철제/스틸": [
        "스테인리스", "스테인레스", "스틸", "steel", "철제", "철재", "금속",
        "분체도장", "알루미늄", "크롬",
    ],
    "플라스틱": ["플라스틱", "abs", "pp수지", "pvc", "아크릴", "폴리프로필렌"],
    "원목": [
        "원목", "고무나무", "러버우드", "자작나무", "소나무", "삼나무",
        "오크원목", "월넛원목", "통원목",
    ],
    "가공목(MDF 외)": [
        "mdf", "lpm", "pb", "e0", "e1", "파티클보드", "무늬목", "합판",
        "hpm", "pet마감", "강화마이카", "집성목", "가공목",
    ],
}
...
ENUM_KEYWORDS: dict[str, dict[str, list[str]]] = {
    ...
    "wood_tone": {
        "밝은 우드톤": ["내추럴우드", "라이트우드", "밝은우드", "메이플", "비치"],
        "중간 우드톤": ["미디엄우드", "티크", "체리"],
...
def clean_product_name(name: str | None) -> str:
    ...
    while tokens and not set("|ㅣ") & set(tokens[0]) and any(
        hint in tokens[0] for hint in _PROMO_HINTS
    ):
        tokens.pop(0)
    cleaned = " ".join(tokens)
