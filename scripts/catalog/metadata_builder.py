"""
원본 데이터 + 규칙으로 상품 메타데이터 초안을 만든다.

오늘의집 원본에는 카테고리, 소재, 크기, 구성품, 옵션처럼
이미 상품에 명시된 정보가 많기 때문에, 이러한 정보까지
VLM으로 다시 추론할 필요가 없다.
...
confidence가 AUTO_ACCEPT 이상이면 자동 채택하고,
기준보다 낮으면 사람이 검수할 대상으로 분류한다.

단, 사람이 직접 확정한 값(source="human")은
항상 최우선으로 취급한다.
"""
...
from app.core import catalog_spec  # noqa: E402
...
EMPTY_SPEC_VALUES = {
    "", "-", "상세페이지 참조", "상세페이지참조", "상세페이지 참고",
    "상세기술서 참조", "상세기술서참조", "상품페이지 참조", "해당사항 없음",
    "해당없음", "별도표기", "상세설명 참조",
}
...
# 일부 상품은 원본의 sub_category 자체가
# catalog_spec에서 사용하는 속성값을 결정할 수 있다.
#
# 예:
#   sub_category = "서랍장" → storage_type = "서랍장"
#
# 이런 경우 상품명이나 VLM으로 추론하지 않고
# 원본 카테고리에서 바로 값을 가져온다.
#
# 구조:
# {
#     "대분류": (
...
SUBCATEGORY_ATTR_MAP: dict[str, tuple[str, dict[str, str]]] = {
    "의자": ("chair_type", {
                    "인테리어의자": "인테리어의자", "스툴·벤치": "스툴·벤치", "빈백": "빈백",
                    "안락의자": "안락의자", "흔들의자": "흔들의자",
                    "학생·사무용의자": "학생·사무용의자", "게이밍의자": "게이밍의자",
                    "바체어": "바체어", "발받침": "발받침",
                }),
    "서랍·수납장": ("storage_type", {
                    "서랍장": "서랍장", "수납장": "수납장", "캐비닛": "캐비닛",
                    "주방수납장": "주방 수납장", "협탁": "협탁",
                }),
...
def spec_value(product: dict, key: str) -> str | None:
    """specifications 값을 읽되 안내 문구는 None으로 본다."""
...
def source_texts(product: dict) -> dict[str, str]:
    ...
    ai_text = " ".join(str(value) for value in (product.get("ai_attributes") or {}).values())
    return {
...
def material_hint(category: str, attribute: str) -> tuple[tuple[str, ...], tuple[str, ...]]:
    ...
    """
    # 프레임 소재를 찾는 경우
    # '프레임', '다리' 등 프레임과 관련된 문장에서 철제/스틸 소재를 우선적으로 선택한다.
    if attribute == "frame_material":
        return ("철제/스틸",), ("프레임", "다리", "각재", "스탠드", "포스트")

    # 상판 소재를 찾는 경우 특정 소재를 무조건 우선하지 않고,
    # '상판' 또는 '탑'이라고 설명된 부분의 소재를 우선해서 찾는다.
    if attribute == "top_material":
        return (), ("상판", "탑")
    # 소파/의자 등 표면 소재가 중요한 카테고리인 경우
    # 프레임 소재보다 실제 겉감(원단/커버/마감재)을 우선한다.
    if category in SURFACE_FIRST_CATEGORIES:
        return SURFACE_MATERIALS, ("마감재", "커버", "원단") # 표면 소재를 찾을 때 확인할 문맥
    # 별도의 소재 우선순위나 문맥이 없는 경우
    return (), ()


def build_draft(product: dict, category: str, sub_category: str | None) -> dict[str, dict[str, Any]]:
    """원본과 규칙만으로 속성 초안을 만든다.
    VLM은 호출하지 않으며, 크롤링된 상품 정보에서
    규칙 기반으로 추출할 수 있는 속성만 먼저 채운다.
    ...
    """
    # 상품명, 옵션, 요약, AI 속성, 소재/구성, 규격 등
    # 원본 데이터에서 필요한 텍스트를 한 번에 정리한다.
    texts = source_texts(product)

    # 상품의 규격/사이즈 문자열을 파싱해서
    # 가로·세로·높이·사이즈 등의 구조화된 값으로 변환한다.
    dimensions = rules.parse_dimensions(texts["spec_size"])

    # 속성을 판단할 때 사용할 원본 정보의 우선순위를 정한다.
    #
    # 같은 속성이 여러 곳에서 발견될 경우
    # 앞에 있는 출처를 우선한다.
    #
    # 우선순위:
    #   1. 상품명          -> 가장 직접적인 상품 정보
    ...
    #원본 데이터에서 어떤 정보를 더 믿을 것인가에 대한 규칙
    ordered_sources: list[tuple[str, str, float]] = [
        ...
        ("specification",
         f"{texts['spec_material']} {texts['spec_component']}", 0.8),
    ]
    # 최종적으로 추출된 속성을 저장할 딕셔너리.
    draft: dict[str, dict[str, Any]] = {}

    def put(key: str, value: Any, source: str, confidence: float) -> None:
    ...
    # --- 원본에 명시된 값 (specifications / options / category_path) ------
    put("color", rules.normalize_color(texts["option"]), "option", 0.9)
    put("color", rules.normalize_color(texts["spec_color"]), "specification",
        0.9)

    if category in SUBCATEGORY_ATTR_MAP and sub_category:
        ...
        put(attr_key,
            ...
            "name", 0.9)
        put(attr_key, mapping.get(sub_category), "category", 0.95)

    # --- 규칙으로 유도되는 값 --------------------------------------------
    put("style", rules.normalize_style(f"{texts['name']} {texts['summary']}"),
        "name", 0.7)
    put("pattern",
    ...
    for key in catalog_spec.attribute_names(category):
        ...
        if key in MATERIAL_ATTRS:
            ...
            # 자재를 4개 이상 나열한 스펙은 대표 소재 판정이 흔들린다.
            chunks = rules.material_chunks(texts["spec_material"])
            put(key,
                rules.normalize_material(
                    texts["spec_material"], category, key,
                    prefer=prefer, focus_words=focus_words,
                ),
                "specification", 0.9 if len(chunks) <= 3 else 0.75)
        elif key == "size":
            put(key, rules.normalize_bed_size(texts["option"]), "option", 0.9)
            put(key, rules.normalize_bed_size(texts["name"]), "name", 0.7)
        elif key == "thickness":
            put(key, rules.normalize_thickness(texts["option"]), "option", 0.9)
            put(key, rules.normalize_thickness(texts["name"]), "name", 0.8)
        elif key in COUNT_ATTRS:
            put(key, rules.normalize_level_count(texts["name"], allowed),
                "name", 0.85)
            put(key, rules.normalize_level_count(texts["option"], allowed),
                "option", 0.85)
        elif key == "seating_capacity":
            put(key, rules.normalize_seating_capacity(texts["name"], allowed),
                "name", 0.8)
        elif key == "length":
            put(key, rules.normalize_length((dimensions or {}).get("width")),
                "derived", 0.85)
        elif allowed == [True, False]:
            # 불리언은 오판 가능성이 가장 커서 자동 채택 기준 아래로 둔다.
            for source, text, _confidence in ordered_sources:
                put(key, rules.normalize_boolean(key, text), source, 0.6)
        else:
            for source, text, confidence in ordered_sources:
                put(key, rules.normalize_enum(key, text, allowed), source,
                    confidence)
...
# 3단계: VLM 보조값 병합
def apply_vlm(draft: dict[str, dict[str, Any]], vlm_attrs: dict[str, Any] | None) -> dict[str, dict[str, Any]]:
    ...
    Returns:
        VLM 값이 병합된 초안입니다.
    """
    # VLM 결과가 아예 없으면 기존 초안을 그대로 반환한다.
    # 불필요하게 복사하거나 병합할 필요가 없다.
    if not vlm_attrs:
        return draft

    # 기존 draft를 복사해서 merged를 만든다.
    # 이렇게 하면 함수 안에서 draft 원본 자체를 수정하지 않는다.
    merged = dict(draft)
    # VLM이 추출한 속성을 하나씩 확인한다.
    #
    # key     : 속성 이름 (예: color, material, shape)
    # payload : 해당 속성의 VLM 결과
    for key, payload in vlm_attrs.items():
        ...
        if key in merged:
            continue
        # VLM 결과가 딕셔너리 형태라면 value를 꺼낸다.
        #
        # 정상적인 VLM 결과:
        # {
        #     "value": "패브릭",
        #     "confidence": 0.9,
        #     "reason": "이미지에서 직물 표면이 확인됨"
        # }
        #
        # 혹시 단순히
        # {"material": "패브릭"}
        ...
        # VLM이 값을 제대로 추출하지 못했다면 저장하지 않는다.
        if value is None:
            continue

        # VLM 값이 원본/규칙 기반 값보다 과도하게 높은 신뢰도를
        # 갖지 않도록 최대값(VLM_CONFIDENCE_CAP)을 제한한다.
        confidence = (
            payload.get("confidence", 0.5) if isinstance(payload, dict) else 0.5
        )
        # VLM에서 새롭게 찾은 속성을 merged에 추가한다.
        merged[key] = {
            "value": value,
            "source": "vlm",
...
# 4단계: 사람 확정값 병합 / 채택 판정
def apply_verified( draft: dict[str, dict[str, Any]], verified: dict[str, Any] | None) -> dict[str, dict[str, Any]]:
    ...
    """
    merged = dict(draft)
    # 사람이 확정한 속성을 하나씩 확인한다.
    #
    # verified가 None이면 빈 딕셔너리로 처리해서
    # 반복문에서 오류가 발생하지 않도록 한다.
    for key, value in (verified or {}).items():
        # 사람이 해당 속성을 None으로 확정했다면
        # 기존에 있던 규칙/VLM 값도 삭제한다.
        #
        # 즉, None은 "값을 모르겠다"가 아니라
        # "이 속성은 사용하지 않는다/값이 없다"는
        # 사람의 최종 판단으로 취급한다.
        if value is None:
            merged.pop(key, None)
            continue
        ...
        # 이 값은 사람이 직접 검수한 값이라는 것을 기록한다.
        merged[key] = {"value": value, "source": "human", "confidence": 1.0}
    # 규칙 + VLM + 사람 확정값이 반영된 최종 결과를 반환한다.
    return merged

# 5단계: 최종 채택 / 검수 대상 분리
def accept( merged: dict[str, dict[str, Any]], category: str) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    """confidence 기준으로 자동 채택값과 검수 대상을 나눈다.
    규칙 + VLM + 사람 확정값까지 병합된 결과를 확인해서
    신뢰도가 충분히 높은 값은 자동으로 채택하고,
    신뢰도가 낮거나 값이 없는 속성은 사람이 검수하도록 분리한다.
    ...
    # 값이 없거나 confidence가 낮아서
    # 사람이 직접 확인해야 하는 속성을 저장한다.
    review: list[dict[str, Any]] = []
    # 해당 카테고리에서 반드시 관리해야 하는
    # 속성 목록을 하나씩 확인한다.
    #
    # 예를 들어 의자라면 color, style, material 등
    # catalog_spec에 정의된 속성을 기준으로 확인한다.
    for key in catalog_spec.attribute_names(category):
        field = merged.get(key)
        # 아예 값이 없다면 자동 채택할 수 없다.
        # → 사람이 직접 값을 확인해야 한다.
        if field is None:
        ...
        # 사람이 직접 확인하도록 review에 넣는다.
            ...
            review.append({
                "attribute": key,
                "value": field["value"],
                "source": field["source"],  # 이 값이 어디서 왔는지 (rule / vlm / human 등)
...
def build_key_features(product: dict) -> list[str]:
    ...
    # 예:
    # "800 x 500 x 750mm"
    # → {"width": 800, "depth": 500, "height": 750}
    size = rules.parse_dimensions(spec_value(product, "크기"))

    # 크기 정보를 정상적으로 파싱했다면
    # 검색하기 좋은 일정한 문장 형태로 변환한다.
    if size:
        features.append(
