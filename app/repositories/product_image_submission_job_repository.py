"""제품 이미지 등록 추천 작업 생성과 큐 처리를 담당하는 Repository입니다."""
...
from app.models.product_image_submission_job import (
    ...
    ProductImageSubmissionJobStatus,
)

_ACTIVE_JOB_CONSTRAINT = "uq_product_image_submission_job_active"
_CLAIM_NEXT_JOB = sqlalchemy.text("""
    ...
    UPDATE product_image_submission_job AS job
    SET status = 'RUNNING',
...
_MARK_JOB_SUCCEEDED = sqlalchemy.text("""
    UPDATE product_image_submission_job
...
class ProductImageSubmissionJobNotFoundError(RuntimeError):
    """요청한 제품 이미지 등록 추천 작업이 존재하지 않는 경우입니다."""


class ActiveProductImageSubmissionJobExistsError(RuntimeError):
    """동일 등록 요청의 활성 작업이 이미 존재하는 경우입니다."""


class ProductImageSubmissionJobStateConflictError(RuntimeError):
...
async def create_job(
    ...
    Returns:
        생성된 ``ProductImageSubmissionJob`` 엔티티입니다.

    Raises:
        ValueError: 최대 실행 횟수가 1보다 작은 경우입니다.
        ActiveProductImageSubmissionJobExistsError: 동일 등록 요청의 활성
            작업이 있는 경우입니다.
        sqlalchemy.exc.IntegrityError: 그 외 DB 무결성 오류가 발생한
            경우입니다.
    """
    if max_attempts < 1:
        raise ValueError("max_attempts must be at least 1")

    job = ProductImageSubmissionJob(
        job_id=uuid.uuid4(),
        submission_id=submission_id,
        status=ProductImageSubmissionJobStatus.PENDING.value,
    ...
    try:
        ...
        if _constraint_name(error) == _ACTIVE_JOB_CONSTRAINT:
            raise ActiveProductImageSubmissionJobExistsError(
                submission_id
            ) from error
        raise
...
async def get_job(
    ...
    """UUID로 제품 이미지 등록 추천 작업 한 건을 조회합니다.

    Args:
        session: 요청 범위의 비동기 SQLAlchemy 세션입니다.
        job_id: 조회할 작업 UUID입니다.

    Returns:
        조회된 ``ProductImageSubmissionJob`` 엔티티입니다.

    Raises:
        ProductImageSubmissionJobNotFoundError: 해당 UUID의 작업이 없는
            경우입니다.
    """
    job = await session.get(ProductImageSubmissionJob, job_id)
    if job is None:
        raise ProductImageSubmissionJobNotFoundError(job_id)
    return job


async def _get_refreshed_job(
    session: AsyncSession, job_id: uuid.UUID
) -> ProductImageSubmissionJob:
    """Raw SQL 상태 변경 이후 최신 작업 상태를 조회합니다."""
    job = await session.get(
        ProductImageSubmissionJob, job_id, populate_existing=True
    )
    if job is None:
        raise ProductImageSubmissionJobNotFoundError(job_id)
...
async def _complete_running_job(
    ...
    if transitioned_job_id is None:
        ...
        raise ProductImageSubmissionJobStateConflictError(
