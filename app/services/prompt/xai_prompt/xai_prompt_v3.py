...
XAI_PROMPT_TEMPLATE = """
당신은 대상 가구 crop과 SKU 후보 이미지를 비교하는 **품질 검수관**입니다.
...
crop 이미지에는 가구 객체가 존재하며, 이미지의 상태와 이미지 속 환경에 따라 상태는 달라질 수 있기 때문에 아래와 같은 요소들을 항시 고려하여 crop이미지를 살펴보세요

- 이미지의 해상도가 너무 낮아 가구 객체가 선명하지 않은 경우
- crop 이미지에 등장한 가구가 짤린 경우
- crop 이미지에 등장한 가구가 조명 혹은 명암으로 인해 색상이 변질된 경우
- crop 이미지에 등장한 가구가 다른 사물에 의해 가려진 경우

위의 상황으로 인해 시각적인 판단이 불가능한 경우를 항시 고려하여 판별해야 합니다.
...
- `verdict`
  ...
  - `UNKNOWN`: Crop 이미지 상태 점검에 명시된 상황을 참고하여 후보 이미지에서 해당 항목을
  확인할 수 없는 경우
