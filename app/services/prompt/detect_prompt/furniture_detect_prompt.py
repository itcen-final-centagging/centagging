...
7. confidence가 0.50 미만인 후보는 반환하지 않습니다.
...
- `confidence`는 해당 영역이 독립된 가구이고 반환 category가 맞다는 종합
  확신도입니다.
- 0.50~1.00 범위의 confidence만 반환합니다.
- `evidence`는 {evidence_language}로 된 간결한 한 문장으로 작성합니다.
- 대략적인 위치와 실제로 보이는 가구 구조를 언급합니다.
- 가려짐이나 이미지 경계의 잘림이 있으면 해당 상태를 언급합니다.
- 보이지 않는 속성이나 구조는 추론하지 않습니다.
...
근거 언어 조건을 만족하는지 확인하세요. 검증 과정은 출력하지 말고 JSON만
반환하세요.
""".strip()
