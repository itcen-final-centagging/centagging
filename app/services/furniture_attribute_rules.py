...
def build_allowed_attribute_schema(category: str) -> dict[str, object]:
    ...
    category_attributes = {
        ...
        in catalog_spec.PRODUCT_ATTRIBUTE[category].items()
    }

    return {
        "category": category,
        "sub_category_options": (
            catalog_spec.PRODUCT_CATEGORY[category]
        ),
        "attributes": {
...
def validate_attribute_result(expected_category: str, result: FurnitureAttributeResult)->FurnitureAttributeResult:
    ...
    normalized_attributes = normalize_attributes(expected_category, result.attributes)

    return result.model_copy(update={"sub_category" : sub_category, "attributes" : normalized_attributes})



def normalize_attributes(category:str, attributes: dict[str, str])->dict[str, str]:
