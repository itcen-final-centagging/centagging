...
async def update_scene_objects(
    ...
    """사용자가 편집한 바운딩 박스와 카테고리를 임시 입력으로 받아 SKU를 추천합니다."""
    try:
        result = await tagging_service.get_sku_candidates(
            scene_id,
            objects=update_request.objects,
        )
...
async def get_recommendation_sku(
    ...
    # 기존 클라이언트의 object_idx 필터링 요청을 서비스에 전달합니다.
