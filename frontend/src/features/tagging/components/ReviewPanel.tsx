...
import { DEFAULT_TAGS } from '@/features/tagging/constants/data';
...
export const ReviewPanel = () => {
  ...
  const [values, setValues] = useState<TaggingValues>({
    category:
      selectedObject?.metadata.category ?? selectedSku?.category ?? '가구',
    ...
    mood:
      selectedObject?.metadata.description ??
      '연출 이미지에서 추출한 가구 정보',
    styleTags: selectedObject?.metadata.keyFeatures.slice(0, 3) ?? DEFAULT_TAGS,
  });

  if (!selectedSku) return null;

  const changeValue = <Key extends keyof TaggingValues>(
    key: Key,
    ...
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(320px,1.12fr)]">
      ...
      <article className="studio-surface p-5">
        ...
        <dl className="mt-4">
          ...
          <MetadataRow label="색상" value={selectedSku.color} />
          <MetadataRow label="소재" value={selectedSku.material} />
          <MetadataRow label="규격" value={selectedSku.size} />
        </dl>
      </article>
      <article className="studio-surface p-5">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3 xl:grid-cols-1">
          <label className="text-sm font-bold text-neutral-700">
            ...
            <select
              ...
              onChange={(event) => changeValue('category', event.target.value)}
          ...
          <label className="text-sm font-bold text-neutral-700">
            ...
            <select
              className="mt-2 h-10 w-full rounded-md border border-neutral-200 bg-white px-3 text-sm font-normal outline-none focus:border-primary focus:ring-3 focus:ring-primary-50"
              onChange={(event) => changeValue('color', event.target.value)}
              value={values.color}
            >
              {colors.map((color) => (
        ...
        <label className="mt-5 block text-sm font-bold text-neutral-700">
          ...
          <textarea
            ...
            maxLength={100}
            onChange={(event) => changeValue('mood', event.target.value)}
            value={values.mood}
          />
        </label>
      </article>
