...
const getRubricScores = (score: number) => {
  const safeScore = score ?? 0;
  const structure = Math.min(30, Math.round(safeScore * 0.3));
  const color = Math.min(30, Math.round(safeScore * 0.29));
  const detail = Math.min(20, Math.round(safeScore * 0.21));
  const context = Math.min(20, safeScore - structure - color - detail);

  return [structure, color, detail, Math.max(0, context)];
};
...
export const RecommendationPanel = () => {
  ...
  const rubricScores = focusedCandidate.rubric
    ? [
    ...
    : getRubricScores(focusedCandidate.score ?? 0);
            ...
            <div className="rounded-xl border border-border p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-extrabold text-text-primary">
                  {focusedCandidate.rubric?.totalScore ??
                    focusedCandidate.score}
                  /100
                </span>
              </div>
              ...
              <p className="mt-4 border-t border-neutral-100 pt-3 text-xs leading-5 text-text-secondary">
                {focusedCandidate.rubric?.xaiReason ??
                  '형태와 소재 특성이 객체 크롭 이미지와 유사하며, 색상과 비율은 검수 단계에서 최종 확인할 수 있습니다.'}
              </p>
            </div>
          </div>
          ...
          <div className="mt-5">
            ...
            <dl className="mt-2 divide-y divide-neutral-100 border-y border-neutral-100">
              {[
                ...
                ['추천 등급', focusedCandidate.grade ?? '검수 필요'],
              ].map(([label, value]) => (
                <div
